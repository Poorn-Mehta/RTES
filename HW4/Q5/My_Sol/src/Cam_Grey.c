...
extern float Grey_Exec_Time[No_of_Frames];
...
static uint8_t Monitor_Q_Setup(void)
{
	...
	if(monitor_queue == -1)
	{
		syslog(LOG_ERR, "<%.6fms>!!Cam_Grey!! Monitor Queue opening Error", Time_Stamp(Mode_ms));
...
void *Cam_Grey_Func(void *para_t)
{
	...
	while(1)
	{
		...
		if(q_recv_resp < 0)
		{
			syslog(LOG_ERR, "<%.6fms>!!Cam_Grey!! Queue Receiving Failed", Time_Stamp(Mode_ms));
		...
		{
			Grey_Exec_Time[framecnt-1] = (us_tstamp2 - us_tstamp1);
		}
	}
}
